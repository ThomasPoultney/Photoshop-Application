...
int main(int argc, char **argv) {
  ...
  // Variables.
  double target_altitude = 4.0;  // The target altitude. Can be changed by the user.
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    const double xlocation = wb_gps_get_values(gps)[0];
    const double ylocation = wb_gps_get_values(gps)[1];
    const double targetXLocation = 20.0;
    const double targetYLocation = 20.0;
